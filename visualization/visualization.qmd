...
## A Story About Climate Change {.smaller}
...
::: notes
The primary graph shows 19 MLS soccer teams and values spent on players salaries. The graph is broken down by several groups: players, players' position. The full length of the bars suggests the total amount spent on players salaries. Individual bars can also be used to compare the relative amount invested in each player within a team. In the sub graph, the data is broken down by player's position and dollar amount spent.
:::
...
## Accessibility & Color
  ...
  -   Colorblindness and complete blindness present accessibility concerns related to visual presentation of information. [@johnson2024]
...
- There are **R** packages that assist visually impaired researchers with developing figures and graphs. [@godfrey2018]
    ...
    + `gridSVG` --- exports graphics using the grid package to SVG, which can be read by a screen reader for the visually impaired. [@murrell]

# Outliers and anomalies

## {.instructor-note}

Can have students simulate colorblindness on visualizations that students previously generated (e.g. using `colorBlindness`) or experiment with simulators online (e.g. [https://www.color-blindness.com/coblis-color-blindness-simulator/](https://www.color-blindness.com/coblis-color-blindness-simulator/))
...
The following four principles out of eight [privacy principles established by the Organization for Economics Co-operation and Development (OECD)](http://oecdprivacy.org/) especially relate to data visualizations. The principle is first described followed by suggestions to comply with it  while creating visualizations. Consider to ask students if they can think of any other practices that will help them comply with these principles.
