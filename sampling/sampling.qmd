...
- Convenience sample, observational data
...
- UC Berkeley was biased
- Confounding or lurker variables (the real explanation, described in the following slides)

This example can also fit into the storytelling and visualization module to highlight the number of different conclusions one may draw from the same (limited) set of data.
...
The previous plots indicate that women tended to apply to departments that had much lower acceptance rate.  There was no discrimination, once accounting for departmental preference.



## The Simpson's Paradox

-   Simpson’s Paradox is a statistical phenomenon in which a trend that appears in different groups tends to reverse or disappear when the groups are combined.
...
- What are the ethical benefits and barriers in running randomized experiments?
  + Results will reflect the population (on average)
  + Previously discussed issues with the ethics of assigning treatments
