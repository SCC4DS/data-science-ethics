...
- consider posing the question: "If researchers determined that more women were hired more overall in comparison to men, would you consider the hiring algorithm to be fair?" 
...
-   In 2015, the Flint Drinking Water Task Force went door-to-door distributing water and asked for volunteering homeowners to submit water samples. 
...
- The initial water water collection and household information came from 156 out of 1951 volunteer households bi-weekly. 

- More household were included in the sentinel sample across time with a total 759 sentinel homes and 4041 non-sentinel. 
...
::: {.column width="60%"}
-   Randomizaiton is a crucial tool: *on average*, treatment group and control group should be similar along important characters (age, race, gender, health etc)

- Without randomziation, the treatment and the control group could yield different outcomes because the groups are fundamentally different (e.g. one group is younger than the other) not because the Tx had any effect.
...
Hypotheticall example: researchers are interested in the effects of caffeine dosage on cyclists performance [@beedie2006].
...
- Assume that the results suggested that cyclists assigned to the moderate caffeine group completed the cycling simulation faster than all others. Can we conclucde that moderate caffeine benefits all cyclists in the population?
...
## 10. The Simpson's Paradox: UC Berkeley Graduate Admissions
...
BGS<- ggplot(df_long, aes(x = Department, y = AdmitRate, fill = Gender)) +
  ...
  labs(
    ...
    subtitle = "Dashed lines indicate otal admission rate by gender)",
    x = "Department",
    y = "Admission Rate"
  ) +
  theme_bw(base_size=16) +
...
## 10. The Simpson's Paradox: UC Berkeley Graduate Gender Discrimination
...
## 10. The Simpson's Paradox: Define
